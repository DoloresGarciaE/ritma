...
6. **La autorización no vive en el middleware** (F0.4). En Next 16 el middleware se llama **Proxy**, corre en Edge y no puede tocar la base: solo hace un chequeo optimista de que exista la cookie de sesión. La guardia real es `requireSession()` en el layout de `(app)`, y el scoping por organización es `withOrg` (F0.6). La fila "Autorización · Middleware + helpers de scoping" de la tabla de arriba se lee así: el middleware redirige, no autoriza.
